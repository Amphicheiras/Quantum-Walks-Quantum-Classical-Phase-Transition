...
%Mend=input('Enter the number of iterations (e.g. 100): ');
N_max=1000;
%epsilon_1=input('Enter the value of epsilon #1 between 0 and 1:');
e_total=1000;
...
for epsilon=1:e_total
    ...
    for N_current=1:N_max
        ...
        a_nk=[0,a_nk_temp(1,1:2*N_max)+((epsilon/e_total)^2)*b_nk_temp(1,1:2*N_max)]/(1+(epsilon/e_total)^2);
        b_nk=[b_nk_temp(1,2:2*N_max+1)+((epsilon/e_total)^2)*a_nk_temp(1,2:2*N_max+1),0]/(1+(epsilon/e_total)^2);
        p_nk(epsilon,:)=a_nk+b_nk;
